...
export default function ProductDetailPage({ params }) {
  ...
  const installFee = installationBreakdown(1, product)
  ...
  const shareUrl = typeof window !== 'undefined'
    ...
    : `https://omniveil.co.ke/products/${product.code}`

  const shareText = product.name + ' from OmniVeil Security at ' + formatKES(product.price)
  const waMsg = 'Hello OmniVeil, I am interested in ' + product.name + ' (' + product.code + ') at ' + formatKES(product.price) + '.'
  const waSupport = 'Hello OmniVeil Support, I have a question about ' + product.name + ' (' + product.code + ').'
    ...
    <div className="bg-[#080c17] text-white min-h-screen">
      ...
      <section className="pt-24 px-6 border-b border-[rgba(0,229,255,0.05)]">
        <div className="max-w-7xl mx-auto py-4 flex items-center gap-3 font-mono text-[10px] tracking-[3px] uppercase flex-wrap">
          <Link href="/" className="text-gray-600 hover:text-[#00e5ff] transition-colors">Home</Link>
          <span className="text-gray-700">/</span>
          <Link href="/products" className="text-gray-600 hover:text-[#00e5ff] transition-colors">Catalogue</Link>
          <span className="text-gray-700">/</span>
            ...
            <div className="border border-[rgba(0,229,255,0.18)] bg-[rgba(0,229,255,0.02)] p-5 mb-6">
              ...
              <div className="flex items-center justify-between gap-4 mb-4 flex-wrap">
                ...
                <div className="font-mono text-[10px] text-gray-500">
                  Install from {formatKES(installFee.base)}
                </div>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <a
                  href={'https://wa.me/' + WHATSAPP_NUMBERS[0].number + '?text=' + encodeURIComponent(waMsg)}
                  target="_blank"
                  rel="noreferrer"
          ...
          <aside className="lg:col-span-5">
            <div className="border border-[rgba(0,229,255,0.15)] bg-[#0b111e]">
              <div className="px-5 py-3 border-b border-[rgba(0,229,255,0.1)] flex items-center justify-between">
                <span className="text-[9px] tracking-[4px] text-[#00e5ff] uppercase font-mono">Specifications</span>
                <span className="text-[9px] tracking-[3px] text-gray-600 uppercase font-mono">{product.code}</span>
              </div>
              <dl className="divide-y divide-[rgba(0,229,255,0.06)]">
                ...
                <SpecRow label="Installation from" value={formatKES(installFee.base)} />
                ...
                >
                  {copied ? (
                    ...
                    <>
                      ...
                      <span className="text-[8px] tracking-[2px] uppercase font-mono">Copy</span>
                    </>
                  )}
                </button>
              </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-px bg-[rgba(0,229,255,0.06)]">
            {BENEFITS.map((b, i) => (
              <div key={i} className="bg-[#0b111e] p-6">
                <div className="text-[10px] tracking-[4px] text-[#00e5ff] uppercase font-mono mb-3">0{i + 1}</div>
                <h3 className="text-white font-black tracking-wide text-base mb-3">{b.title}</h3>
      ...
      <section className="px-6 pb-16">
        <div className="max-w-7xl mx-auto border border-[rgba(0,229,255,0.18)] bg-[rgba(0,229,255,0.03)] p-8 grid grid-cols-1 md:grid-cols-12 gap-6 items-center">
          <div className="md:col-span-7">
            <div className="text-[10px] tracking-[5px] text-[#00e5ff] uppercase font-mono mb-3">Need Help?</div>
            <h3 className="text-2xl md:text-3xl font-black text-white tracking-tight mb-2">Talk to our team</h3>
          ...
          <div className="md:col-span-5 grid grid-cols-1 sm:grid-cols-2 gap-3">
            <a
              href={'https://wa.me/' + WHATSAPP_NUMBERS[1].number + '?text=' + encodeURIComponent(waSupport)}
              target="_blank"
              rel="noreferrer"
            ...
            >
              <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor">
                <path d="M20.52 3.48A11.77 11.77 0 0 0 12.06 0C5.5 0 .16 5.34.16 11.9c0 2.1.55 4.15 1.6 5.96L0 24l6.3-1.65a11.85 11.85 0 0 0 5.76 1.47h.01c6.56 0 11.9-5.34 11.9-11.9 0-3.18-1.24-6.17-3.45-8.44Z" />
              </svg>
              WhatsApp Support
            </a>
            <a
        ...
        <section className="px-6 py-14 border-t border-[rgba(0,229,255,0.08)] bg-[#06090f]">
          <div className="max-w-7xl mx-auto">
            <div className="flex items-end justify-between mb-8 gap-6 flex-wrap">
              <div>
                <div className="text-[10px] tracking-[5px] text-[#00e5ff] uppercase font-mono mb-3">You might also like</div>
                <h2 className="text-2xl md:text-3xl font-black tracking-tight text-white">Featured Items</h2>
              </div>
              <Link href="/products" className="font-mono text-[10px] tracking-[3px] uppercase text-gray-500 hover:text-[#00e5ff] transition-colors">
                View all
              </Link>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-px bg-[rgba(0,229,255,0.06)]">
              {related.map((r) => (
                ...
                >
                  <div className="relative h-44 overflow-hidden">
                    ...
                    />
                    <div className="absolute inset-0" style={{ background: 'linear-gradient(to top, #080c17, rgba(8,12,23,0.2), transparent)' }} />
                    <div className="absolute top-3 left-3 font-mono text-[9px] text-[#00e5ff] opacity-70 tracking-widest">{r.code}</div>
                  </div>
                  <div className="p-5">
                    <div className="font-mono text-[9px] text-gray-500 tracking-[3px] mb-2">{r.category}</div>
                    <h3 className="font-black text-sm tracking-wide text-white mb-2 line-clamp-2">{r.name}</h3>
                    <div className="font-mono text-[12px] text-[#00e5ff] tracking-[2px]">{formatKES(r.price)}</div>
                  </div>
                </Link>
              ))}
            </div>
...
function MetaCell({ label, value }) {
  return (
    <div className="bg-[#080c17] px-4 py-3">
      <div className="text-[8px] tracking-[3px] text-gray-600 uppercase font-mono mb-1">{label}</div>
      <div className="text-[11px] text-gray-200 font-mono tracking-[1px] capitalize">{value}</div>
    </div>
  )
}
...
function SectionHeader({ eyebrow, title, className = '' }) {
  return (
    <div className={`mb-6 ${className}`}>
      <div className="text-[9px] tracking-[5px] text-[#00e5ff] uppercase font-mono mb-3">{eyebrow}</div>
...
function SpecRow({ label, value }) {
  return (
    <div className="flex items-center justify-between px-5 py-3">
      <dt className="text-[10px] tracking-[2px] text-gray-500 uppercase font-mono">{label}</dt>
      <dd className="text-[13px] text-gray-200 font-mono tracking-[1px] text-right capitalize">{value}</dd>
    </div>
  )
}
